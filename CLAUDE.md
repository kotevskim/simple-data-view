...
- Column behavior is controlled via SQL alias suffixes: `_display` (paired: hidden companion replacing cell content with raw HTML, original used for sort/filter; standalone: renders directly as raw HTML column), `_nosort` (disable sorting), `_nofilter` (disable filtering). Suffixes are combinable in any order and stripped from the display name. Parsing lives in `parseColumnName()` in `server.js`.
