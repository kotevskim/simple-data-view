...
- Column behavior is controlled via SQL alias suffixes: `_html` (raw HTML), `_nosort` (disable sorting), `_nofilter` (disable filtering). Suffixes are combinable in any order and stripped from the display name. Parsing lives in `parseColumnName()` in `server.js`.
